...
### 3. 当某一列的值全是 NULL 时，`count(col)` 返回 0 但 `sum(col)` 返回 NULL
...
### 5. 代码中写分页查询逻辑时，若 count 为 0 应直接返回
...
### 12. 因国际化需要，所有的字符存储与表示，均采用 `utf8mb4` 字符集
...
### 13. `TRUNCATE TABLE` 比 `DELETE` 速度快
...
### 2. POJO 类的布尔属性不能加 `is`，而数据库字段必须加 `is_`
...
### 4. `sql.xml` 配置参数使用 `#{}`，不要使用 `${}`

> [!CAUTION] 强制
> `sql.xml` 配置参数使用：`#{}`，`#param#` 不要使用 `${}`，此种方式容易出现 SQL 注入。

### 5. iBATIS 自带的 `queryForList` 不推荐使用
...
### 7. 更新数据表记录时，必须同时更新 `update_time` 字段值为当前时间
...
### 10. `<isEqual>` 中的 `compareValue` 是与属性值对比的常量
